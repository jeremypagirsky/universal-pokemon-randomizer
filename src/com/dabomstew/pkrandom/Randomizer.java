...
import com.dabomstew.pkrandom.InvalidSupplementFilesException.Type;
...
public class Randomizer {
  ...
  public int randomize(final String filename, final OutputStream logStream, long seed) {
    ...
    // Movesets
    boolean noBrokenMoves = !settings.isAllowBrokenMoves();
    ...
    if (settings.getTrainersMod() == Settings.TrainersMod.RANDOM) {
      romHandler.randomizeTrainerPokes(
          ...
          !settings.isTrainersCanUseLegendaries(),
          !settings.isTrainersEarlyWonderGuard());
    ...
    switch(settings.getWildPokemonMod()) {
      case RANDOM:
        romHandler.randomEncounters(settings.isUseTimeBasedEncounters(),
            ...
            !settings.isEncounterLegendaries());
        break;
      case AREA_MAPPING:
    ...
    if (settings.getInGameTradesMod() == Settings.InGameTradesMod.RANDOMIZE_GIVEN) {
      romHandler.randomizeIngameTrades(false,
          settings.getNicknames(),
      ...
      romHandler.randomizeIngameTrades(true,
          settings.getNicknames(),
          settings.isRandomizeInGameTradesNicknames(),
          settings.getTrainerNames(),
          settings.isRandomizeInGameTradesOTs(),
    ...
    if (romHandler.canChangeStarters()) {
      if (settings.getStartersMod() == Settings.StartersMod.CUSTOM) {
        ...
        for (int i = 0; i < starterCount; i++) {
          ...
          log.println("Set starter " + (i + 1) + " to "
              + pkmn.name);
          starters.add(pkmn);
        }
  ...
  private int maybeChangeAndLogStaticPokemon(
      final PrintStream log, final RomHandler romHandler, boolean raceMode, int checkValue) {
